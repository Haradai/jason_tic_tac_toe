...
//Lets assume that mysymbol is true if we have X and false if we have o
mysymbol :- symbol(x)
...
//At the start of each turn, check the available cells and get how many. Then perform some strategy.
+round(Z) : next <- !get_available_cells;
					L = .length(AvailableCells);
					!strategy.


//Typical actions
+!get_available_cells <- .findall(available(X,Y),available(X,Y),AvailableCells).

+!play_available_cell <- .nth(N,AvailableCells,available(A,B));
							play(A,B).


//We are the first ones! Grab the center
+!strategy : L==9 <- play(1,1).

//There is already one piece and its in the center
+!strategy : (L==8 & isCenter(X,Y)) <- 
